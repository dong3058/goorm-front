        //이 3개는 env 파일로 따로 분리해야될듯 api key.
        const REST_API_KEY = 'aad9fffaff5b6655b991487c5ff8b445';
        const sk_app_key="NjRgcEPAUm1f5Y2XCrQ7A8XJOeTx2OGy3D872r2U"    
        const serviceKey="njQB8V0%2Bf8iukC%2BuEysH0INTK2nGufDu5YFcjfyiAr2IRBYV8OiyTW4wX8NUQ%2BUHrBu0R6fw5mdjmgtRiqd89Q%3D%3D"
        ...
        var btn=document.getElementById("area_btn");//애같은경우 사용자가 정한 사용자위치를 바꾸는 버튼이다.
        ...
        function async1(){
            ...
            geocoder.coord2Address(parseInt(origin_cord[1],10),parseInt(origin_cord[0],10),callbacks);



            function getmarker(origin_name:string){
           
            let serviceKey="njQB8V0%2Bf8iukC%2BuEysH0INTK2nGufDu5YFcjfyiAr2IRBYV8OiyTW4wX8NUQ%2BUHrBu0R6fw5mdjmgtRiqd89Q%3D%3D"
            let date=new Date();
            ...
            var callback =function(result:any, status:any) {
                if (status === kakao.maps.services.Status.OK) {
                    ...
                    ps.categorySearch('BK9', placesSearchCB, {useMapCenter:true,
                        radius:500});//반경 500 내에서 카테고리가 은행인 애들 찾는것. 
                    ...
                    async function placesSearchCB (data:any, status:any, pagination:any) {
                        ...
                        if (status === kakao.maps.services.Status.OK) {
                            ...
                            /* 대충 plcae_Data안에는 이런 객체들이 배열꼴로 들어가있다.
                            {
                            address_name: "서울 노원구 중계동 584-1"
                            ...
                            id: "20550377"
                            phone: "02-952-2742"
                            place_name: "KB국민은행 상계역지점"
                            place_url: "http://place.map.kakao.com/20550377"
                            road_address_name: "서울 노원구 한글비석로 383"
                            x: "127.072939655145"
                            y: "37.6592432341433"}*/
                            finally{
        ...
        async function async2(){
            ...
            for(const x of marker_save_map.keys()){
                console.log("keys:",x);
            }
            for(const position of datafromback){
                ...
                try {
                    ...
                    testroute.forEach((router,index)=>{
                        console.log(router,index);
                            if((router.geometry ?? {}).type==="LineString"){

                            router.geometry.coordinates.forEach((vertex,idx)=>{
        ...
        function makemarker(pos_data:any,linepath:any){
            ...
            for(let i=0;i<3;i++){
            hexcolorcode+=Math.floor(Math.random() * (256 - 0) +10+i).toString(16);
            }       
