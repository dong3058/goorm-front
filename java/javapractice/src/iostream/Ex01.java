...
public class Ex01 {
    ...
    public static void fileInputStrmEx1 () {
        ...
        try (FileInputStream fis = new FileInputStream(SONG_PATH)) {
            ...
            while ((readByte = fis.read()) != -1) {
                //(readByte = fis.read()이렇게대입하는 과정도 대입하고나서 그값을 반환한다.
    ...
    public static void fileInputStrmEx3 () {
        ...
        try (FileInputStream fis = new FileInputStream(SONG_PATH)) {
            ...
            while ((readByte = fis.read(buffer)) != -1) {//읽어올대 buffer라는 곳에 채워서 돌려준다
                //즉 한번에 buffr크기만큼 받아온다는것이다.

                //  💡 byte[]로부터 지정된 범위와 인코딩의 문자열 생성
                String readStr = new String(
                        buffer, 0, readByte, charset
                );//두번쨰 인자 0 은 시작 포인트를 의미.
    ...
    public static void bufferedInputEx () {
        ...
        try (
            ...
            //  - 2. 바구니 크기보다 적게 남았을 때 그만큼만 담아옴 즉 빈공간을 0 으로채우지않는다.
