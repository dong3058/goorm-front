...
public class Main {
    public static void main(String[] args) {
        ...
        Random random = new Random();//임포트 받아야되므로 lang에 속한 개 아님.

        //  아래를 여러 차례 실행해 볼 것

        //  ⭐ 아래를 주석해제한 뒤 실행해 볼 것
        //  random.setSeed(1234); 시드값 설정. 즉 랜덤 테이블을 정한다는 의미이다.

        int randInt1 = random.nextInt();
        int randInt2 = random.nextInt();
        int randInt3 = random.nextInt();
        // 범위 지정 (이상, 미만)
        int randInt4 = random.nextInt(0, 10);//9~0의숫자 즉 10은 제외.
        ...
        // 범위 지정 (이상, 미만)
        double randDbl3 = random.nextDouble(3.14, 5.67);//애도 마찬가지이다.
