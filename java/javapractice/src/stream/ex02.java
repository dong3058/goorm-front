...
public class ex02 {
    ...
    //Object[] ifReuse = fromArray.toArray();이미 스트림을 한번썻으니까.
    //즉 물이 이미 흘러서 사라졌다는말.
    ...
    IntStream fromIntAry = Arrays.stream(intAry);//stream<integer>와는 다른 클래스이다.
...
f
    ...
    Stream<Integer> withIter1 = Stream
            .iterate(0, i -> i + 2)//첫번쨰인자는 초기값,두번쨰는 unaryoperator.이다
            .limit(10);
    ...
    Stream<Integer> fromRangeBox = fromRange1.boxed();//원시자료형을 래퍼클래스로 바구는것.
    ...
    IntStream randomInts = new Random().ints(5, 0, 100);//스트림크기,초기값,랜덤범ㅁ위
    //여기선 0~100사이.
