package thread.threadpoll.competablefutrue;
...
public class Main {
    ...
    public static void thenAcceptEx1 () throws ExecutionException, InterruptedException {
        ...
        //  💡 thenAccept : 받아온 값을 Consumer로 실행 여기선 위의 gethell를 받아서 처리한다이말.
        ...
        //printHello.get(); // ⭐ 활성화하고 나서 재실행 왜냐하면 get으로 값을 받아야 thenaccept도 실행이되니까.
        //단 gethello안의 값 받아오기나 return 문 같은경우는 비동기적이므로 다른 스레드에서 실행이되고있다.

        //즉 스트림처럼 completablefutre도 함수혀을 프로그래밍이 가능하다는말이다.
    }
    ...
    public static void thenAcceptEx2 () throws ExecutionException, InterruptedException {
        ...
        //  ⭐️ 비동기 메소드 체이닝
        //보이드인 이유는 then accpet라는 consumer즉 void르르 반환하는 애를 받기떄문이다.
    ...
    public static void exceptionallyEx (Boolean error) throws ExecutionException, InterruptedException {
        CompletableFuture.supplyAsync(() -> {
            ...
            //  💡 exceptionally : 오류 발생시 대체 값 반환 즉 future값을 오류가 발생하면 "안녕못해"로돌려준다믄말.
            e.printStackTrace();
            return "안녕 못해.";
        }).thenApply(s -> {
            takeTime(false);
    ...
    public static void thenComposeEx () throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<Swordman> getBlueChamp = getChamp(Side.BLUE);//애내둘은 비동기적 실행
        //즉 다른 메서드ㅔ서 실행되므로 둘 포함 양진영검사훈련중은 서로 제각각의 타이밍에 나온다.
        ...
        getBlueChamp.thenCompose(
                ...
                //두개의 future결과물을 조합한다.다르게 말하면 2개가다끝나야 진행이된다이말.
                )
    ...
    public static void allOfEx1 () throws ExecutionException, InterruptedException {
        ...
        CompletableFuture.allOf(
                roll1, roll2, roll3, roll4, roll5//즉 이거는 위의 5개애들이 결정될떄까지 대기한다는말이다.
        ).thenRun(() -> {
            //  💡 프린트 순서 확인
